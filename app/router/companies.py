...
from schemas.companies import CompanyRequest,CompanyResponse,CompanyUpdate, StatusResponse, StatusRequest, CompanyBranchRequest,\
CompanyBranchResponse, CompanyBranchUpdate
from dbservice import db
from models.company import Company, Status, Branch
...
def register_comapny(company: CompanyRequest):
    try:
        ...
        return {"message":"Company successfully registered, kindly wait for verifiction!"}
...
def update_company_details(update_details: CompanyUpdate, company_id:int):
    try:
        ...
        if update_details.is_active:
            company.is_active = update_details.is_active
...
def update_branch_status(branch_id: int, branch_update: CompanyBranchUpdate):
    try:
        ...
        if branch_update:
            branch.is_active==branch_update
